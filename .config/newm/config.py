...
## Output / Monitors ───────────────────────────────────────────────────────
outputs = [
	##-- Default (All Monitors), Mode: 1920x1080, Scale: 1
    { 'name': '*', 'scale': 1.0, 'width': 1920, 'height': 1080,
...
def app_rules(view):
    ...
    float_apps = ("io.calamares.calamares", "Yad", "yad", "nm-connection-editor", "pavucontrol", 
    ...
    #blur_apps = ("Alacritty", "kitty", "foot", "rofi", "Rofi", "waybar")  # applications that should have the blur effect
    ...
    ##-- Open kitty in floating, with blur effect.
...
panels = {
    ...
    # Fixed issue with waybar (credit: hofmann.christian)
	'top_bar': {
	'native': {
		'enabled': True,
		'height': 38,
...
term_kitty = '~/.config/newm/scripts/kitty'
...
rofi_bluetooth = '~/.config/newm/scripts/rofi_bluetooth'
...
rofi_asroot = '~/.config/newm/scripts/rofi_asroot'
...
def key_bindings(layout: Layout) -> list[tuple[str, Callable[[], Any]]]:
    return [
		...
		# -- Terminal : kitty
        #("L-Return", lambda: os.system(f"{term_kitty} &")),
        #("L-S-Return", lambda: os.system(f"{term_kitty} -f &")),
        #("L-A-Return", lambda: os.system(f"{term_kitty} -F &")),
        ...
        ("A-F2", lambda: os.system(f"{rofi_runner} &")),
        ("L-d", lambda: os.system(f"{rofi_launcher} &")),
        ("L-m", lambda: os.system(f"{rofi_music} &")),
        ("L-b", lambda: os.system(f"{rofi_bluetooth} &")),
        ("L-n", lambda: os.system(f"{rofi_network} &")),
        ("L-x", lambda: os.system(f"{rofi_powermenu} &")),
        ("L-r", lambda: os.system(f"{rofi_asroot} &")),
        ("L-s", lambda: os.system(f"{rofi_screenshot} &")),

		# -- Wofi
        #("L-d", lambda: os.system(f"{wofi_menu} &")),
        #("A-F1", lambda: os.system(f"{wofi_menu} &")),
        #("L-x", lambda: os.system(f"{wofi_powermenu} &")),
